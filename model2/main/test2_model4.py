#Response 3 from chatgpt to implement paper(with saving the model)
import torch
...
class IRSN(nn.Module):
    def __init__(self, num_classes):
        ...
        # General feature extractor (CLIP Vision Encoder)
        ...
        # Item-specific encoders (for head, top, bottom, shoes)
...
# Placeholder for item masks (Replace with CLIPSeg or other segmentation models)
def generate_item_masks(batch):
    # Example: Generate dummy masks for demonstration
    masks = {
        "head": torch.ones_like(batch[:, 0:1]),
        "top": torch.ones_like(batch[:, 0:1]),
        "bottom": torch.ones_like(batch[:, 0:1]),
        "shoes": torch.ones_like(batch[:, 0:1]),
    }
    return masks
...
model = IRSN(num_classes=len(train_dataset.classes))
model.to(device)
